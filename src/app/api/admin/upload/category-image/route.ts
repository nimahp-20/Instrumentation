...
async function uploadCategoryImageHandler(request: AuthenticatedRequest) {
  try {
    ...
    const { url, mime } = await validateAndSaveAdminImageFile(entry, 'categories');
...
export const POST = withAdminAuth(uploadCategoryImageHandler);
